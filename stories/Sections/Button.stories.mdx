...
Chatwave comes with predefined set of colors for buttons. The color of the button can be set with `color-scheme` to `woot-button`.
...
Chatwave currently use [IonIcons version 2](https://ionic.io/ionicons/v2) which are font-icons. The button position is fixed on the left.
