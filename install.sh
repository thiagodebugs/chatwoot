...
CHATWOOT_HUB_URL="https://hub.2.chat.singularmodel.com.br/events"
...
function setup_chatwoot() {
  ...
  git clone https://github.com/thiagodebugs/chatwoot.git
  cd chatwoot
  git checkout "$BRANCH"
  bundle
  yarn
...
function setup_ssl() {
  ...
  wget https://raw.githubusercontent.com/thiagodebugs/chatwoot/master/deployment/nginx_chatwoot.conf
...
Join the community at https://chat.singularmodel.com.br/community?utm_source=cwctl
***************************************************************************
...
function get_cw_version() {
  CW_VERSION=$(curl -s https://chat.singularmodel.com.br/api | python3 -c 'import sys,json;data=json.loads(sys.stdin.read()); print(data["version"])')
}
...
https://www.chat.singularmodel.com.br/docs/deployment/deploy-chatwoot-in-linux-vm?utm_source=cwctl
...
Get help, https://chat.singularmodel.com.br/community?utm_source=cwctl
...
function cwctl_upgrade_check() {
    ...
    local remote_version_url="https://raw.githubusercontent.com/thiagodebugs/chatwoot/master/VERSION_CWCTL"
